...
public class BandNameGenerator {
   ...
   */
  public void loadNouns() { this.nouns = loadTxt(this.nounsFile); }

  /**
   * Return a string that capitalizes the first letter of s and lowercases the
   * rest. So for example, capitalizeFirst('cat') returns 'Cat', and
   * capitalizeFirst('CHUBBY CAt') returns 'Chubby cat'. Read the assignment
   * carefully and think about how to implement this part of the code.
   *
   * @param s
   * @return
   */
   ...
   * Make sure to return "UNINITIALIZED" if the band name generator is not
   * initialized correctly yet.
   * @return the generated name or "UNINITIALIZED"
   */
  public String generateName() {
    int lenAdj = this.adjectives.length;
    int lenNoun = this.nouns.length;
    String name;
    int indAdj1 = (int)Math.floor(Math.random() * lenAdj);
    int indAdj2 = (int)Math.floor(Math.random() * lenAdj);
    while (indAdj2 == indAdj1) {
      indAdj2 = (int)Math.floor(Math.random() * lenAdj);
    }
    int indNoun = (int)Math.floor(Math.random() * lenNoun);
    name = capitalizeFirst(this.adjectives[indAdj1].toLowerCase()) + " " +
           capitalizeFirst(this.adjectives[indAdj2].toLowerCase()) + " " +
           capitalizeFirst(this.nouns[indNoun].toLowerCase());
    return name;
  }
   ...
   * @return
   */
  public String[] getAdjectives() { return adjectives; }
  ...
  public static void main(String[] args) {
    ...
    BandNameGenerator generator = new BandNameGenerator("nouns.txt", "adjectives.txt");
    generator.loadNouns();
